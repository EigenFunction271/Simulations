...
def run_simulation(N, connectivity, num_trials=15):
    """
    ...
    networks = {
        ...
        'Watts-Strogatz': lambda N, p: generate_watts_strogatz_network(N, k=max(2, N//4), p=p),
...
def main():
    # Parameters
    N_values = range(3, 26)  # Network sizes from 3 to 25
    connectivity = 0.2  # Fixed connectivity value
    ...
    # Add a central title
    fig.suptitle(f'Message Spread vs Network Size\nConnectivity={connectivity}, {num_trials} trials', fontsize=14, y=0.95)
    ...
    for i, N in enumerate(N_values):
        print(f"Running simulations for N={N} ({i+1}/{len(N_values)})")
    ...
    for idx, (name, data) in enumerate(results.items()):
        ...
        ax.plot(N_values, data, linestyle='-', linewidth=2, label='Simulation')
        
        # Add theoretical curve for Erdős-Rényi
        if name == 'Erdős-Rényi':
            theoretical = [N * (1 + (N-1) * connectivity) for N in N_values]
            ax.plot(N_values, theoretical, '--', color='gray', linewidth=2, label='Theoretical')
